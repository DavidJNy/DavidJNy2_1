...
const WebSocketComponent = ({ endpoint, title }) => {
  ...
  useEffect(() => {
    ...
    if (filteredData.length > 0) {
      ...
      const sortedKeys = columnOrderRef.current.filter(k => keys.includes(k))
          ...
          >
            {columns.map(col => (
              <li key={col} className="mb-1">
                <button
                  className={`btn btn-sm w-100 text-start ${hiddenCols.has(col) ? "btn-secondary" : "btn-primary"}`}
            ...
            <li>
              <button
                className={`btn btn-sm w-100 text-start ${voiceEnabled ? "btn-success" : "btn-outline-light"}`}
